...
func (s *ProjectUpdateService) GetProjectUpdatesByNGO(ctx context.Context, ngoID primitive.ObjectID) ([]models.ProjectUpdate, error) {
	var updates []models.ProjectUpdate
	cursor, err := s.collection.Find(ctx, bson.M{"ngo_id": ngoID})
	if err != nil {
		utils.LogError(err, fmt.Sprintf("Failed to get project updates for NGO %s", ngoID.Hex()))
		return nil, err
	}
	defer cursor.Close(ctx)

	if err = cursor.All(ctx, &updates); err != nil {
		utils.LogError(err, "Failed to decode project updates for NGO")
		return nil, err
	}
	return updates, nil
}
